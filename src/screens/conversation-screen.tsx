...
export default function ConversationScreen() {
  ...
  const conversation = (Array.isArray(conversations.data) ? conversations.data : [])
    .find((item: Conversation) => item.id === id);
  const otherParticipants = conversation?.participants
    .filter((participant) => userId(participant) !== Number(currentId)) || [];
  ...
  const attachmentMutation = useMutation({
    mutationFn: (selection: AttachmentSelection) => sendMessageAttachment({
      conversationId: id,
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_, message) => {
      updateCachedMessage(deletedMessage({ ...message, attachments: [] }));
  ...
  const confirmDelete = (message: Message) => {
    ...
    showConfirm(
      'Delete message?',
      'This removes the message and all attachments for everyone, but keeps a deleted-message marker.',
                ...
                >
                  {item.reply_preview ? (
                    <View style={[styles.replyPreview, mine && styles.replyPreviewMine]}>
                      <Text numberOfLines={1} style={[styles.replySender, mine && styles.mineMeta]}>
                        {item.reply_preview.is_deleted ? 'Deleted message' : item.reply_preview.sender.user_name || item.reply_preview.sender.email}
                      </Text>
                      <Text numberOfLines={2} style={[styles.replyText, mine && styles.mineMeta]}>
                        {item.reply_preview.is_deleted ? 'This message was deleted.' : item.reply_preview.content || attachmentSummary(item.reply_preview.attachments)}
                      </Text>
                    </View>
                  ) : null}
                  {!deleted ? <MessageAttachments attachments={item.attachments} mine={mine} /> : null}
                  {item.content || deleted ? (
                    <Text style={[styles.message, mine && styles.mineText, deleted && styles.deletedText]}>
      ...
      {editing || replyingTo ? (
        <View style={styles.composerContext}>
          ...
          <View style={styles.contextCopy}>
            <Text style={styles.contextTitle}>{editing ? 'Editing message' : `Replying to ${senderName(replyingTo)}`}</Text>
            <Text numberOfLines={1} style={styles.contextText}>{editing?.content || replyingTo?.content || attachmentSummary((editing || replyingTo)?.attachments)}</Text>
          </View>
          <Pressable accessibilityLabel="Cancel message action" onPress={cancelComposerMode} style={styles.contextClose}>
            <X color={colors.muted} size={18} />
          </Pressable>
        </View>
      ) : null}
      <View style={styles.composer}>
        <MessageAttachmentPicker disabled={busy || Boolean(editing)} onSelect={(selection) => attachmentMutation.mutate(selection)} />
        <TextInput
          editable={!busy}
          maxLength={5000}
          multiline
      ...
      <Modal animationType="fade" onRequestClose={() => setActionMessage(null)} transparent visible={Boolean(actionMessage)}>
        <Pressable onPress={() => setActionMessage(null)} style={styles.modalBackdrop}>
          <Pressable onPress={(event) => event.stopPropagation()} style={styles.actionSheet}>
            ...
            {actionMessage ? (
              <>
                <Pressable onPress={() => beginReply(actionMessage)} style={styles.actionRow}>
                  <Reply color={colors.primary} size={20} /><Text style={styles.actionText}>Reply</Text>
                </Pressable>
                {userId(actionMessage.sender) === Number(currentId) ? (
                  <>
                    <Pressable disabled={!actionMessage.content} onPress={() => beginEdit(actionMessage)} style={styles.actionRow}>
                      <Pencil color={actionMessage.content ? colors.primary : colors.muted} size={20} /><Text style={styles.actionText}>Edit message</Text>
                    </Pressable>
                    <Pressable onPress={() => confirmDelete(actionMessage)} style={styles.actionRow}>
                      <Trash2 color={colors.danger} size={20} /><Text style={[styles.actionText, styles.dangerText]}>Delete for everyone</Text>
                    </Pressable>
                  </>
                ) : null}
              </>
...
const styles = StyleSheet.create({
  ...
  list: { padding: spacing.lg, flexGrow: 1, justifyContent: 'flex-end', paddingBottom: 24 },
  ...
  mineText: { color: '#fff' },
  deletedText: { fontStyle: 'italic', opacity: .78 },
  ...
  replyPreviewMine: { borderLeftColor: '#fff', backgroundColor: 'rgba(255,255,255,.14)' },
  ...
  composer: { flexDirection: 'row', alignItems: 'flex-end', gap: 8, padding: 12, backgroundColor: '#fff', borderTopWidth: 1, borderTopColor: colors.border },
  ...
  modalBackdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: 'rgba(15,23,42,.46)' },
