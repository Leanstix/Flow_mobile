...
function conversationPreview(conversation: Conversation, previewsEnabled: boolean) {
  const lastMessage = conversation.last_message;
  if (!lastMessage) return 'Start the conversation';
  if (!previewsEnabled) return 'Message preview hidden';
  if (lastMessage.is_deleted || lastMessage.deleted_at) return 'Message deleted';
  return lastMessage.content || 'Start the conversation';
}

export default function MessagesScreen() {
  const currentId = useAuthStore((state) => state.session?.user.user_id || state.session?.user.id);
  const previews = usePreferencesStore((state) => state.messagePreviews);
  ...
  const other = (item: Conversation): User | undefined => item.participants.find((participant) => (participant.id || participant.user_id) !== currentId);

  return (
    <View style={styles.root}>
      <View style={styles.header}>
        <View style={{ flex: 1 }}><Text style={styles.title}>Messages</Text><Text style={styles.subtitle}>Private realtime conversations with your classmates.</Text></View>
        <Pressable accessibilityLabel="Open calls" onPress={() => router.push('/calls')} style={styles.callButton}><Phone color={colors.primary} size={20} /></Pressable>
      </View>
      <FlatList
        ...
        renderItem={({ item }) => {
          const participant = other(item);
          return (
            <Pressable onPress={() => router.push({ pathname: '/conversation/[id]', params: { id: String(item.id), name: item.name } })}>
              <Card style={styles.row}>
                <Avatar user={participant} size={52} />
                <View style={{ flex: 1 }}>
                  <View style={styles.nameRow}><Text style={styles.name}>{item.name || participant?.user_name || participant?.email || 'Conversation'}</Text>{item.unread_count > 0 ? <View style={styles.badge}><Text style={styles.badgeText}>{item.unread_count}</Text></View> : null}</View>
                  <Text numberOfLines={1} style={styles.preview}>{conversationPreview(item, previews)}</Text>
                </View>
              </Card>
            </Pressable>
          );
        ...
        ListEmptyComponent={<Text style={styles.empty}>No conversations yet. Find a student in Explore to start one.</Text>}
      />
    </View>
  );
}
