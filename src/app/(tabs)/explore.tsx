...
export default function ExploreScreen() {
  ...
  const [mode, setMode] = useState<Mode>(initial.startsWith('#') ? 'posts' : 'people');

  useEffect(() => {
    if (typeof params.q !== 'string') return;
    setQuery(params.q);
    if (params.q.startsWith('#')) setMode('posts');
    if (params.q.startsWith('@')) setMode('people');
  }, [params.q]);
