...
function ThreadNode({ node, onReply }: { node: CommentNode; onReply: (comment: Comment) => void }) {
  return (
    <View style={{ marginLeft: cappedIndent(node.depth) }}>
      <View style={styles.comment}>
        <Avatar user={node.user} size={36} />
        <View style={{ flex: 1 }}>
          {node.parent_preview ? <Text numberOfLines={1} style={styles.parentPreview}>Replying to @{node.parent_preview.user.user_name || 'student'}</Text> : null}
          <Text style={styles.commentName}>{node.user.user_name || node.user.email}</Text>
          <RichText mentions={node.mentions || []} style={styles.commentText} value={node.content} />
          <Pressable onPress={() => onReply(node)}><Text style={styles.reply}>Reply</Text></Pressable>
        </View>
      </View>
      {node.children.map((child) => <ThreadNode key={child.id} node={child} onReply={onReply} />)}
    </View>
  );
}
...
function RootThread({ root, onReply }: { root: Comment; onReply: (comment: Comment) => void }) {
  ...
  return (
    <View style={styles.thread}>
      <ThreadNode node={{ ...(tree[0] || root), children: open ? tree[0]?.children || [] : [] }} onReply={onReply} />
      {root.replies_count > 0 ? <Pressable onPress={() => setOpen((value) => !value)} style={styles.threadToggle}>{open ? <ChevronUp color={colors.primary} size={16} /> : <ChevronDown color={colors.primary} size={16} />}<Text style={styles.threadToggleText}>{open ? 'Hide thread' : `View ${root.replies_count} ${root.replies_count === 1 ? 'reply' : 'replies'}`}</Text></Pressable> : null}
      {open && replies.isLoading ? <Text style={styles.loading}>Loading thread…</Text> : null}
    </View>
  );
}
...
export default function DiscussionScreen() {
  ...
  const submitMutation = useMutation({
    mutationFn: (text: string) => replying ? replyToComment(replying.id, text) : addComment(id, text),
    onSuccess: (created) => {
      setContent('');
      const rootId = created.root || created.id;
      setReplying(null);
      client.invalidateQueries({ queryKey: ['comments', id] });
      client.invalidateQueries({ queryKey: ['comment-thread', rootId] });
      client.invalidateQueries({ queryKey: ['feed'] });
    },
    onError: (error) => showApiError(error, 'Could not publish comment'),
  });
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} keyboardVerticalOffset={Platform.OS === 'ios' ? 90 : 0} style={styles.root}>
      <FlatList
        automaticallyAdjustKeyboardInsets
        contentContainerStyle={styles.list}
        data={unwrapList(query.data)}
        keyboardDismissMode={Platform.OS === 'ios' ? 'interactive' : 'on-drag'}
        keyboardShouldPersistTaps="handled"
        keyExtractor={(item) => String(item.id)}
        renderItem={({ item }) => <RootThread root={item} onReply={startReply} />}
        ItemSeparatorComponent={() => <View style={{ height: 12 }} />}
        ListEmptyComponent={<Text style={styles.empty}>No comments yet. Add the first response.</Text>}
      />
      <View style={styles.composer}>
        {replying ? <View style={styles.replying}><View><Text style={styles.replyingText}>Replying to @{replying.user.user_name || 'student'}</Text><Text numberOfLines={1} style={styles.replyingPreview}>{replying.content}</Text></View><Pressable onPress={() => { setReplying(null); setContent(''); }}><Text style={styles.cancel}>Cancel</Text></Pressable></View> : null}
