...
export function PostCard({ post }: { post: Post }) {
  ...
  const like = useMutation({ mutationFn: () => toggleLike(post.id), onMutate: async () => {
    const update = (value: any): any => Array.isArray(value) ? value.map((item) => item.id === post.id ? { ...item, has_liked: !item.has_liked, likes_count: item.likes_count + (item.has_liked ? -1 : 1) } : item) : value?.pages ? { ...value, pages: value.pages.map(update) } : value?.results ? { ...value, results: update(value.results) } : value;
    queryClient.setQueriesData({ queryKey: ['feed'] }, update);
  }, onError: (error) => showApiError(error, 'Could not update like'), onSettled: () => queryClient.invalidateQueries({ queryKey: ['feed'] }) });
  const repostMutation = useMutation({ mutationFn: () => repost(post.id), onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['feed'] }); showSuccess('Reposted', 'The post is now visible on your profile.'); }, onError: (error) => showApiError(error, 'Could not repost') });
